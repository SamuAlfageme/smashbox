...
do_not_report_as_failure("Some tests cases are failing due to Non-deterministic behaviour with the sync client on windows and files differing by case #5870 ")
...
def creator(step):
    ...
    if (N != nfiles):
        ...
        error_check(len(lost_files) == 0,
                    "There are %d files lost. The filenames are %s " % (len(lost_files), str(lost_files)))
        extra_files = list(filter(lambda x: x not in conflict_cases and x not in lost_files, files))
        error_check(len(extra_files) == 0,
                    "There are %d extra files. The filenames are %s" % (len(extra_files), str(extra_files)))
